...
final class Graph: NSObject {
    var nodes: [GraphNode] = []
    var neighbors: [GraphNeighborNode] = []
    
    func addNode(node: GraphNode) {
        nodes.append(node)
    }
    
    func addNeighbor(firstNode: GraphNode, secondNode: GraphNode, cost: Double) {
        let neighbor1: GraphNeighborNode = .init(first: firstNode, second: secondNode, cost: cost)
        let neighbor2: GraphNeighborNode = .init(first: secondNode, second: firstNode, cost: cost)
...
extension Graph {
    ...
    func findSafetyNode() -> GraphSafetyNode? {
        if nodes.count == 1 {
...
private extension Graph {
    func getCost(firstNode: GraphNode, secondNode: GraphNode) -> Double? {
    ...
    func isValidCostBetweenNeighbors(firstNode: GraphNode, secondNode: GraphNode) -> Bool {
